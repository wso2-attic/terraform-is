...
variable "wso2_tags" {
  description = "Two node Identity Server HA setup with backend mysql database"
  type        = map
...
resource "azurerm_virtual_machine_scale_set" "wso2_scale_set" {
  ...
  network_profile {
    name    = "is-scaleset-nps"
    primary = true

    ip_configuration {
      name                                   = "is-scaleset-ips"
...
resource "azurerm_lb_rule" "wso2_lb_portal_rule" {
  ...
  frontend_port                  = "9443"
  backend_port                   = "9443"
  frontend_ip_configuration_name = "islb-fip"
